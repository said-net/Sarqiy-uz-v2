...
import { Button, Chip, Dialog, DialogBody, DialogFooter, DialogHeader, IconButton, Spinner } from "@material-tailwind/react";
import { BiMenu, BiXCircle } from "react-icons/bi";
import { setRefresh } from "../managers/refresh.manager";

function GetAdminPays() {
    ...
    useEffect(() => {
        setIsLoad(false);
        axios(`${API_LINK}/boss/get-new-admin-pays`, {
            headers: {
    ...
    function setStatus(st) {
        if (st === 'reject') {
            axios.post(`${API_LINK}/boss/set-status-admin-pay`, { status: 'reject', id: select?.id }, {
                headers: {
                    'x-auth-token': `Bearer ${localStorage.getItem('access')}`,
                }
            }).then((res) => {
            ...
            axios.post(`${API_LINK}/boss/set-status-admin-pay`, { status: 'success', id: select?.id }, {
                headers: {
                    'x-auth-token': `Bearer ${localStorage.getItem('access')}`,
                }
            }).then((res) => {
            ...
            <div className="flex items-center justify-normal flex-col p-[5px] ">
                <div className="flex items-center justify-between w-full h-[70px] shadow-sm bg-white  border-b p-[0_5px]">
                    <div className="flex items-center justify-between">
                        ...
                        <p className="w-[140px] text-center border-x h-[70px] flex items-center justify-center text-[13px]">ADMIN ID</p>
                            ...
                            <div key={i} className="flex items-center justify-between w-full h-[70px] shadow-sm bg-white  border-b p-[0_5px]">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="w-[140px] text-center border-x h-[70px] flex items-center justify-center text-[13px]">
                                        <Chip color="red" value={String(p?.user_id)} />
                                    </div>
            ...
            <Dialog open={select?.id !== ''} size="xxl" className="flex items-center justify-center bg-[#1b424a80] backdrop-blur-md">
                <div className="flex items-center justify-start flex-col md:w-[700px] w-[90%] p-[10px] bg-white shadow-lg rounded-md">
                    <DialogHeader className="w-full relative flex items-center justify-between">
                        <p className="text-[20px]">Admin uchun to'lov</p>
                    ...
                    <DialogBody className="w-full border-y">
                        <p className="text-black">Admin ID: {select?.user_id}</p>
                        <p className="text-black">Admin: {select?.name}</p>
                        <p className="text-black">Raqami: {select?.phone}</p>
