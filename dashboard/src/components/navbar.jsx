    ...
    return (
        <>
            ...
            <div className={`flex items-center justify-start flex-col w-[300px] h-[100vh] bg-white duration-500 shadow-md p-[10px] overflow-y-scroll fixed top-0 ${open ? 'left-0' : 'left-[-300px]'} md:relative md:top-auto md:left-auto z-[998] ${path === '/print-cheques' || path === '/wait-delivery' ? 'hidden' : ''}`}>
                ...
                {/*  */}
                <Link onClick={() => setOpen(false)} to='/competitions' className={classLink + `${path === '/competitions' && 'bg-gradient-to-r from-orange-500 to-red-500 text-white'}`}>
                    <BiGift className="mr-[10px]" />
                    Konkurslar
                    <span className="absolute right-[10px] rounded-full p-[5px] bg-[#fff0]">{stats?.comps}</span>
                </Link>
                {/*  */}
