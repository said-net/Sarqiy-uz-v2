...
function Navbar() {
    ...
    const [stats, setStats] = useState({ products: 0, categories: 0, operators: 0, wait_delivery: 0, sended: 0, reject: 0, delivered: 0, archive: 0, wait: 0, neworders: 0, inoperator: 0, users: 0, couriers: 0, oper_pays: 0, race: 0, history_orders: 0, owners: 0, main: 0 });
    ...
    return (
        <>
            ...
            <div className={`flex items-center justify-start flex-col w-[300px] h-[100vh] bg-white duration-500 shadow-md p-[10px] overflow-y-scroll fixed top-0 ${open ? 'left-0' : 'left-[-300px]'} md:relative md:top-auto md:left-auto z-[998] ${path === '/print-cheques' || path === '/wait-delivery' ? 'hidden' : ''}`}>
                ...
                {/*  */}
                <Link onClick={() => setOpen(false)} to='/main' className={classLink + `${path === '/main' && 'bg-gradient-to-r from-orange-500 to-red-500 text-white'}`}>
                    <BiCategory className="mr-[10px]" />
                    Reklama
                    <span className="absolute right-[10px] rounded-full p-[5px] bg-[#fff0]">{stats?.main}</span>
                </Link>
                {/*  */}
