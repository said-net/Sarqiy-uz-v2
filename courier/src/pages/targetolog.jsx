...
import { Button, Checkbox, Dialog, DialogBody, DialogFooter, DialogHeader, IconButton, Input, Option, Select, Spinner } from "@material-tailwind/react";
import { BiBox, BiLocationPlus, BiMoney, BiPencil, BiQuestionMark, BiSearch, BiSolidTruck } from 'react-icons/bi';
import Regions from '../components/regions.json';
function Targetolog() {
    const [orders, setOrders] = useState([]);
    const [isLoad, setIsLoad] = useState(false);
    const [refresh, setRefresh] = useState(false);
    const [id, setId] = useState(localStorage.getItem('target_id'))
    ...
    return (
        <div className="flex items-start justify-center flex-wrap w-full">
            <div className="flex items-center justify-center w-full h-[50px] bg-white rounded px-[10px] relative">
                <Input label="Targetolog ID raqamini kiriting!" value={id} onChange={e => setId(e.target.value)} color="red" type="number" />
                <div className="absolute right-[10px]">
                    <IconButton onClick={() => setRefresh(!refresh)} color="red" className="rounded">
                        <BiSearch />
                    </IconButton>
                </div>
            </div>
                ...
                <DialogBody>
                    ...
                    {edit?.status === "success" &&
                        <>
                            ...
                            <div className="flex items-center justify-center w-full mb-[10px]">
                                <Input variant="standard" label="Hudud haqida batafsil" required type="text" onChange={e => setEdit({ ...edit, city: e.target.value })} value={edit?.city} icon={<BiLocationPlus />} />
                            </div>
                            <div className="flex items-center justify-center w-full mb-[10px]">
