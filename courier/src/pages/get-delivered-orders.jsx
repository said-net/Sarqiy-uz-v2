...
function GetDeliveredOrders() {
    ...
    useEffect(() => {
        setIsLoad(false);
        axios(`${API_LINK}/courier/get-delivered-orders`, {
            headers: {
                                ...
                                return (
                                    <div key={i} className="flex items-start justify-start flex-col w-[100%] sm:w-[300px] bg-white shadow-sm p-[10px] rounded m-[5px] relative">
                                        ...
                                        <div className="flex mt-[20px] items-center justify-center bottom-[10px] w-full left-[0]">
                                            <Button onClick={() => setSelected({ id: o?.id, _id: o?._id, product: o?.product, courier_comment: o?.courier_comment, status: '', recontact: '' })} disabled color="green" className="rounded w-[100%]">YETKAZILGAN</Button>
                                        </div>
                                    </div>
                                )
                            })
