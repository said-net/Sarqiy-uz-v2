...
function Navbar() {
    ...
    useEffect(() => {
        axios(`${API_LINK}/courier/get-stats`, {
            headers: {
    ...
    return (
        <>
            <div className={`flex items-center justify-start flex-col w-[300px] rounded shadow-sm bg-white h-[100vh] duration-500 xl:relative fixed xl:top-auto top-0 xl:left-auto ${open ? 'left-0' : 'left-[-300px]'} z-[999] p-[10px]`}>
                <div className={`duration-500 fixed flex xl:hidden top-[10px] ${open ? 'left-[250px]' : 'left-[10px]'} z-[998] bg-red-500 rounded-full`}>
                ...
                {/*  */}
                <Link to='/rejected-orders' className={`flex items-center justify-start w-full text-[20px] rounded p-[5px] ${path === '/rejected-orders' ? 'bg-gradient-to-r from-red-400 to-orange-500 text-white' : 'text-blue-gray-400'} relative mb-[10px]`}>
